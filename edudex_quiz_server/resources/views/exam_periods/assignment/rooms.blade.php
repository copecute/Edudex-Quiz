        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @else
                    <form id="assignmentForm" action="{{ route('exam-periods.assignment.rooms.store', $examPeriod) }}" method="POST">
                        @csrf
                        @foreach($shifts as $shift)
                                    ...
                                    <div class="col-md-3 mb-3">
                                        <div class="card">
                                            <div class="card-body p-2">
                                                ...
                                                <select class="form-select form-select-sm subject-select mb-2"
                                                        data-room-index="{{ $loop->parent->index }}_{{ $loop->index }}"
                                                        {{ !$shift->rooms->contains($room->id) ? 'disabled' : '' }}>
                                                    <option value="">Chọn môn thi</option>
                                                    @foreach($shift->subjects as $subject)
                                                        @php
                                                            $roomInShift = $shift->rooms->firstWhere('id', $room->id);
                                                            $isSelected = $roomInShift && $roomInShift->pivot->exam_period_subject_id == $subject->id;
                                                        @endphp
                                                ...
                                                <select class="form-select form-select-sm proctor-select"
                                                    ...
                                                    @foreach($proctors as $proctor)
                                                        @php
                                                            $roomInShift = $shift->rooms->firstWhere('id', $room->id);
                                                            $isSelected = $roomInShift && $roomInShift->pivot->exam_period_proctor_id == $proctor->proctor_id;
                                                        @endphp
                                                        <option value="{{ $proctor->proctor_id }}" {{ $isSelected ? 'selected' : '' }}>
                                                            {{ $proctor->username }} - {{ $proctor->fullName }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
...
$(document).ready(function() {
    ...
    $(document).on('change', '.subject-select', function() {
        ...
        const roomCapacity = parseInt($(this).closest('.card-body').find('.room-checkbox').data('capacity'));
    ...
    function updateTotalCapacityInfo() {
        ...
        Object.entries(subjectStudents).forEach(([subjectId, students]) => {
            ...
            if (capacity < students) {
                const subject = $(`option[value="${subjectId}"]`).first().text().split('(')[0].trim();
        ...
        $('.card.mb-4').each(function() {
            ...
            $(this).find('.proctor-select').each(function() {
                ...
                if (proctorId && assignedProctors.has(proctorId)) {
                    alert('Một cán bộ coi thi không thể coi nhiều phòng trong cùng một ca thi');
                    hasError = true;
                    return false;
                }
                if (proctorId) {
        ...
        $('.card.mb-4').each(function() {
            ...
            $(this).find('.room-checkbox:checked').each(function() {
                ...
                rooms.push({
                    ...
                    proctor_id: $(this).closest('.card-body').find('.proctor-select').val() || null
                });
            });
