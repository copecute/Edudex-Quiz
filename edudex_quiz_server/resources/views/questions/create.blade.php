                            ...
                            <div class="col-md-8">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="url" class="form-control @error('link_media') is-invalid @enderror" 
                                           id="link_media" name="link_media" value="{{ old('link_media') }}"
                                        ...
                                        <div class="answer-group mb-3 border rounded p-3">
                                            <div class="form-check mb-2">
                                                <input class="form-check-input" type="radio" name="correct_answer" 
                                                       value="{{ $i }}" {{ old('correct_answer') == $i ? 'checked' : '' }}
                                                       required>
                                                <label class="form-check-label">Là đáp án đúng</label>
                                            </div>
                                            <div class="mb-2">
                                                <input type="text" class="form-control @error('answers.'.$i.'.content') is-invalid @enderror" 
                                                       name="answers[{{ $i }}][content]" 
                                                       value="{{ old('answers.'.$i.'.content') }}"
                                                       placeholder="Nội dung đáp án {{ $i + 1 }}" required>
                                                @error('answers.'.$i.'.content')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            <div>
                                                <input type="url" class="form-control @error('answers.'.$i.'.link_media') is-invalid @enderror" 
                                                       name="answers[{{ $i }}][link_media]"
                                                       value="{{ old('answers.'.$i.'.link_media') }}"
                                                       placeholder="Link media cho đáp án {{ $i + 1 }}">
                                                @error('answers.'.$i.'.link_media')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        </div>
                            ...
                            <div class="col-md-4">
                                    ...
                                    <select class="form-select @error('subject_code') is-invalid @enderror" 
                                        ...
                                        @foreach($subjects as $subject)
                                            <option value="{{ $subject->code }}" 
                                                {{ old('subject_code') == $subject->code ? 'selected' : '' }}>
                                                {{ $subject->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('tags') is-invalid @enderror" 
                                        ...
                                        @if(old('subject_code'))
                                            @foreach(App\Models\Tag::where('subject_code', old('subject_code'))->get() as $tag)
