...
@section('title', 'Chỉnh sửa môn thi')
        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Chỉnh sửa môn thi</h5>
                </div>

                <div class="card-body">
                    <form action="{{ route('exam-period-subjects.update', ['examPeriod' => $examPeriod->id, 'examPeriodSubject' => $examPeriodSubject->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <select class="form-select @error('subject_id') is-invalid @enderror" 
                                ...
                                @foreach($subjects as $subject)
                                    <option value="{{ $subject->id }}" 
                                            {{ old('subject_id', $examPeriodSubject->subject_id) == $subject->id ? 'selected' : '' }}>
                                        {{ $subject->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-select @error('exam_id') is-invalid @enderror" 
                                ...
                                @foreach($exams as $exam)
                                    <option value="{{ $exam->id }}" 
                                            {{ old('exam_id', $examPeriodSubject->exam_id) == $exam->id ? 'selected' : '' }}>
                                        {{ $exam->name }}
                                    </option>
                                @endforeach
                            </select>
...
$(document).ready(function() {
    ...
    $('#subject_id').on('change', function() {
        ...
        const oldExamId = '{{ old('exam_id', $examPeriodSubject->exam_id) }}';
